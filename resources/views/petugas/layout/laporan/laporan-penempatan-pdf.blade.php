...
<head>
    ...
    <title>Laporan Data Penempatan PDF</title>
    <style>
        .page-break {
...
<body>
    @foreach ($penempatans as $penempatan)
        ...
        <p style="margin-top: -20px; margin-left: 150px;">:HD3000 (Dept. Pendidikan dan Pelatihan)</p> --}}
            <p style="margin-left: -55px;">NO PENEMPATAN</p>
            <p style="margin-top: -20px; margin-left: 150px;">: {{ $penempatan->no_penempatan }}</p>
            <p style="margin-left: -55px;">TANGGAL PENEMPATAN</p>
            <p style="margin-top: -20px; margin-left: 150px;">: {{ $penempatan->tanggal_penempatan }}</p>
        ...
        @php
            $count = DB::table('detail_penempatans')
                ->select('*')
                // ->join('pengadaans', 'pengadaans.no_pengadaan', '=', 'detail_barangs.no_pengadaan')
                ->where('no_penempatan', $penempatan->no_penempatan)
                // ->groupBy('no_ruangan')
                ->count();
        @endphp
        @if ($count > 4)
